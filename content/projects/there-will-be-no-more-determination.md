...
The technical core is a **GAN pipeline** trained around **military helicopter video** from **Afghanistan, Iraq, Pakistan**, and comparable contexts. The network learns from patterns in that material, then **generates images** that inherit its textures and motion logic without reproducing a clean “scene.” The point is not to celebrate the model, but to use its capacity for **semantic drift** as a mirror: the same statistical machinery that can produce compelling images is also continuous with the abstraction that makes violence easier to administer at distance.

## Context

The work was developed during **Con.divisione Residenza 2022** in **Maul**, as a period of focused production around the project’s dataset, training runs, and how the outputs should be shown in space.
