...
export function ContentJsonViewer({ json }: { json: string }) {
  return (
    <div className="flex min-h-0 flex-col overflow-hidden border border-(--index-divider) border-dotted bg-(--foreground)/[0.02]">
      ...
      <textarea
        className="min-h-[min(50dvh,28rem)] w-full flex-1 resize-y border-0 bg-transparent p-4 font-mono text-(--foreground)/85 text-[11px] leading-relaxed outline-none md:min-h-[32rem] md:p-5 md:text-xs"
        readOnly
        spellCheck={false}
        value={json}
      />
